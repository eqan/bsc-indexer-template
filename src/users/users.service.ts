...
export class UsersService {
     ...
     * @return Users
     */
    async createUser(createUserInput: CreateUserInput | CreateUserOnLoginInput): Promise<Users> {
      try{
   ...
   */
    async loginUser(loginUserInput: LoginUserInput): Promise<{access_token: String}>{
      ...
      if (!user) {
        throw SystemErrors.LOGIN_AUTHORIZATION;
        // throw new UnauthorizedException(`User address didnt match the signer address`);
      } else {
